...
class Invaders(GameApp):
    ...
    _scoretext: The currently active message for score.
               [GLabel, or None if there is no message to display.]
    _alien_speed: The speed at which the alien moves
    """
    ...
    def start(self):
        ...
        self._text = GLabel(x=GAME_WIDTH/2, y=GAME_HEIGHT/2, width=10,
        height=10, font_size=36,font_name="RetroGame.ttf",
    ...
    def update(self,dt):
        ...
        if self._state == STATE_NEWWAVE:
            self._wave = Wave()
            self._text = GLabel(x=GAME_WIDTH*7/8,y=GAME_HEIGHT*0.95,width=40,height=80,
            font_size=25,font_name="TimesBold.ttf",text="SHIP LIFE: "+str(self._wave.getlife()))
            self._scoretext = GLabel(x=GAME_WIDTH/8,y=GAME_HEIGHT*0.95,width=40,height=80,
            font_size=25,font_name="TimesBold.ttf",text="SCORE: " + str(self._wave.getscore()))
            self._state = STATE_ACTIVE

        elif self._state == STATE_ACTIVE:
            self.game_over()
            self._scoretext.text = "SCORE: " + str(self._wave.getscore())
            self._wave.update(self.input,dt)

            if self._wave.verify_ship_collide() == True:
                self._state = STATE_PAUSED

        elif self._state == STATE_CONTINUE:
            self._text =self._text = GLabel(x=GAME_WIDTH*7/8,y=GAME_HEIGHT*0.95,width=40,height=80,
            font_size=25,font_name="TimesBold.ttf",text="SHIP LIFE: " + str(self._wave.getlife()))
            self._wave.revive()
            self._state = STATE_ACTIVE
    ...
    def draw(self):
        ...
        if self._scoretext is not None:
    ...
    def inactive_to_newwave(self):
        """
        Changes the value of _state from STATE_INACTIVE to STATE_NEWWAVE

        This method first checks if the key that is held down is 's', then it
        assigns the attribute _state to STATE_NEWWAVE.
        """
        key = self.input.is_key_down('spacebar')
        if key == True:
            self._state = STATE_NEWWAVE

    def paused(self):
        """
        This method changes the _state attribute to STATE_PAUSED

        This method sets the state to STATE_PAUSED and sets _text to a GLabel
        object.
        """
        self._text = GLabel(x=GAME_WIDTH/2, y=GAME_HEIGHT/2, width=10,
        height=10, font_size=36,font_name="RetroGame.ttf",
    ...
    def paused_to_continue(self):
        """
        This method sets the state to STATE_CONTINUE.

        When the keypress 'spacebar' is detected, the attribute _state is set to
        STATE_CONTINUE
        """

        keypress = self.input.is_key_down('spacebar')
        if keypress == True:
    ...
    def game_over(self):
        """
        This method is responsible for creating text for losing and winning,
        based on lifecount, if all aliens are destroyed,
        or if the aliens reach the dline.
        """
        if self._wave.getlife() < 0 or self._wave.alien_wins():
            self._state = STATE_COMPLETE
            self._text = GLabel(x=GAME_WIDTH/2, y=GAME_HEIGHT/2,
             width=10, height=10, font_size=36,
            font_name="RetroGame.ttf", text="You Lost!")

        if self._wave.player_wins() == True:
            self._state = STATE_COMPLETE
            if self._wave.getlife() >= 0 and self._wave.player_wins():
                self._text = GLabel(x=GAME_WIDTH/2, y=GAME_HEIGHT/2,
                width=10, height=10, font_size=36,
                font_name="RetroGame.ttf", text="You Win!")
